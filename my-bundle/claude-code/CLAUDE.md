...
**Актуализация** — команда `/generate`: переизучает проект, пере-выводит inputs и карту структуры, после подтверждения перегенерирует конфигурацию. Если файлы агента удалены — `kit-setup generate .aikit/manifest.json` восстановит их из манифеста (память не теряется).
