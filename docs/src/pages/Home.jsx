...
import { LogoIcon, GitHubIcon, FeatureCard, StepCard } from '../ui'
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3">
            {h.runners.items.map((item) => (
              <div
                key={item.id}
                className="card-glass px-5 py-3.5 flex items-center gap-3"
              >
                <span className="font-medium text-gray-900 dark:text-white text-sm">{item.name}</span>
                <span className="text-xs text-gray-400 dark:text-gray-500 border-l border-gray-200 dark:border-white/10 pl-3">
                  {item.note}
                </span>
              </div>
            ))}
