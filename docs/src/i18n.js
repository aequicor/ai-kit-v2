  ...
  <other-agent>/...`,
    ...
    claude: {
      ...
      bundles: {
        ...
        items: [
          { title: 'my-bundle@0.4.0 (remote)', body: 'Universal constructor fetched straight from the AI-Kit repository (source: "remote"). Targets Claude Code and Codex. CLAUDE.md/AGENTS.md are assembled from technology toggles (KMP, Android, coroutines, Compose, Decompose, Ktor, tests) plus a manifest-backed project profile that survives regeneration; each technology brings a deep SDK skill, with cross-cutting security-audit, leak-hunt, perf-optimize and kotlin-build skills and the ai-kit ops skill on top. Ships /remember (persist a project fact into the manifest) and /generate (re-study the project and refresh the profile + inputs) commands.' },
  ...
  <другой-агент>/...`,
    ...
    claude: {
      ...
      bundles: {
        ...
        title: 'С чего начать',
        items: [
          { title: 'my-bundle@0.4.0 (remote)', body: 'Универсальный конструктор прямо из репозитория AI-Kit (source: "remote"). Таргеты Claude Code и Codex. CLAUDE.md/AGENTS.md собираются из тумблеров по технологиям (KMP, Android, корутины, Compose, Decompose, Ktor, тесты) + профиль проекта в манифесте, переживающий перегенерацию; каждая технология включает глубокий SDK-скилл, сверху — кросс-катовые security-audit, leak-hunt, perf-optimize, kotlin-build и ops-скилл ai-kit. Команды: /remember — запомнить проектный факт в манифесте, /generate — переизучить проект и актуализировать профиль + inputs.' },
