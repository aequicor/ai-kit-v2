  ...
  <other-agent>/...`,
    ...
    claude: {
      hero: {
        ...
        lead: 'Supported targets: Claude Code (CLAUDE.md, .claude/agents/, .claude/commands/, .claude/skills/, .claude/hooks/, .mcp.json), OpenAI Codex (AGENTS.md, .codex/config.toml, .codex/agents/*.toml, .codex/prompts/), OpenCode and Qwen Code.',
      },
      files: {
        eyebrow: 'Artifacts',
      ...
      bundles: {
        ...
        items: [
          { title: 'my-bundle@0.4.1 (remote)', body: 'Universal constructor fetched straight from the AI-Kit repository (source: "remote"). Targets Claude Code and Codex. CLAUDE.md/AGENTS.md are assembled from technology toggles (KMP, Android, coroutines, Compose, Decompose, Ktor, tests) plus a manifest-backed project profile that survives regeneration; each technology brings a deep SDK skill, with cross-cutting security-audit, leak-hunt, perf-optimize and kotlin-build skills and the ai-kit ops skill on top. Includes guidance for keeping documentation aligned with public behavior. Ships /remember (persist a project fact into the manifest) and /generate (re-study the project and refresh the profile + inputs) commands.' },
  ...
  <другой-агент>/...`,
    ...
    claude: {
      hero: {
        ...
        lead: 'Поддерживаемые таргеты: Claude Code (CLAUDE.md, .claude/agents/, .claude/commands/, .claude/skills/, .claude/hooks/, .mcp.json), OpenAI Codex (AGENTS.md, .codex/config.toml, .codex/agents/*.toml, .codex/prompts/), OpenCode и Qwen Code.',
      },
      files: {
        eyebrow: 'Артефакты',
      ...
      bundles: {
        ...
        title: 'С чего начать',
        items: [
          { title: 'my-bundle@0.4.1 (remote)', body: 'Универсальный конструктор прямо из репозитория AI-Kit (source: "remote"). Таргеты Claude Code и Codex. CLAUDE.md/AGENTS.md собираются из тумблеров по технологиям (KMP, Android, корутины, Compose, Decompose, Ktor, тесты) + профиль проекта в манифесте, переживающий перегенерацию; каждая технология включает глубокий SDK-скилл, сверху — кросс-катовые security-audit, leak-hunt, perf-optimize, kotlin-build и ops-скилл ai-kit. Включает правила написания и актуализации документации вместе с публичным поведением. Команды: /remember — запомнить проектный факт в манифесте, /generate — переизучить проект и актуализировать профиль + inputs.' },
